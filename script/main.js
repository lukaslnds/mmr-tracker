...
function setCowTracker() {
    if (document.getElementById('cowtracker').checked) {
        for (var j = 0; j < 35; j++) {
            document.getElementById("dungeon" + j).style.visibility = "hidden";
        }
        for (var j = 35; j < 39; j++) {
...
function setLongJump(sender){
 ...
 function setRecoilFlipIce(sender){
 if (sender.checked){
        RecoilFlipIce = true
    }else {
        RecoilFlipIce = false
 ...
 if (sender.checked){
   ...
   function setBigOctoSkip(sender){
 if (sender.checked){
        BigOctoSkip = true
    }else {
        BigOctoSkip = false
 ...
 if (sender.checked){
     ...
     function setTerminaStump(sender){
 if (sender.checked){
        TerminaStump = true
    }else {
        TerminaStump = false
...
function getHudPanel(rowIndex) {
    ...
    if (!panels.length) {
        var defs = [
            ['hud-objects', 'Objects', '⚔'],
            ['hud-trade', 'Trade Items', '◆'],
            ['hud-chants', 'Chants', '♪'],
            ['hud-masks', 'Masks', '🎭'],
            ['hud-equipment', 'Equipment', '🛡'],
            ['hud-remnants', 'Remnants', '✦']
        ];
        defs.forEach(function(def) {
            var panel = document.createElement('section');
            panel.className = 'hud-panel ' + def[0];
            panel.setAttribute('data-title', def[1]);
            panel.setAttribute('data-icon', def[2]);
            sender.appendChild(panel);
        });
        panels = sender.querySelectorAll('.hud-panel');
    }
    var panelIndex;
    if (rowIndex <= 1) panelIndex = 0;
    else if (rowIndex === 2) panelIndex = 1;
    else if (rowIndex >= 3 && rowIndex <= 4) panelIndex = 4;
...
function addItemRow() {
    ...
    itemGrid[r]['tablerow'].className = 'hud-item-row hud-row-' + r;
    itemGrid[r]['tablerow'].appendChild(itemGrid[r]['row']);
    getHudPanel(r).appendChild(itemGrid[r]['tablerow']);
...
function removeItemRow() {
    ...
    itemGrid[r]['tablerow'].parentNode.removeChild(itemGrid[r]['tablerow'])
    itemGrid.splice(r, 1);
